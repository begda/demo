...
let createRoutes = function () {
	...
	isM().map((item, index) => {
			...
			dd2 = `async (resolve)=>{
                ...
                if(os){
                    ...
                    //require(['@/views/${keys.join("/")}.vue'], resolve)
                    	module=	await import('@/views/${keys.join("/")}.vue')
											module.default.name='home'
											console.log(await import('@/views/home/index.vue'))
		...
		const route = {name: '', path: '', component: dd2,mobile:item.mobile,file:keys.join("/")};
		let parent = routes
		// 按分割好的路径数组循环
		keys.forEach((key, i) => {
